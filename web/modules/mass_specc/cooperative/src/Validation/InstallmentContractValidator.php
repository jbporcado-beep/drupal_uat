...
class InstallmentContractValidator {
    ...
    public function validate(InstallmentContractDto $dto, array &$errors, int $row_number): void {
        ...
        if (empty($payment_periodicity)) {
            $errors[] = "Row $row_number | FIELD 'PAYMENT PERIODICITY' IS MANDATORY";
        }
        if (!empty($payment_periodicity) && !in_array($payment_periodicity, self::PAYMENT_PERIODICITY_DOMAIN)) {
            $errors[] = "Row $row_number | 10-029: FIELD 'PAYMENT PERIODICITY' IS NOT CORRECT";
        }

        if (strlen($installments_number) === 0) {
            $errors[] = "Row $row_number | 10-030: FIELD 'INSTALLMENTS NUMBER' IS MANDATORY";
        }
        if (!ctype_digit($installments_number) || (int) $installments_number <= 0 || strlen($installments_number) > 3) {
            $errors[] = "Row $row_number | 10-059: FIELD 'INSTALLMENTS NUMBER' IS NOT NUMERIC OR LENGTH IS NOT CORRECT";
        }

        if (strlen($financed_amount) === 0) {
            $errors[] = "Row $row_number | 10-161: FIELD 'FINANCED AMOUNT' IS MANDATORY";
        }
        if (!ctype_digit($financed_amount) || (int) $financed_amount <= 0 || strlen($financed_amount) > 15) {
            $errors[] = "Row $row_number | 10-058: FIELD 'FINANCED AMOUNT' IS NOT NUMERIC OR LENGTH IS NOT CORRECT";
        }

        if (!ctype_digit($monthly_payment_amount) || (int) $monthly_payment_amount <= 0 || strlen($monthly_payment_amount) > 15) {
            $errors[] = "Row $row_number | 10-060: FIELD 'MONTHLY PAYMENT AMOUNT' IS NOT NUMERIC OR LENGTH IS NOT CORRECT";
        }
        if (in_array($contract_phase, ['AC', 'CL', 'CA']) && (empty($monthly_payment_amount) || (int) $monthly_payment_amount === 0)) {
            $errors[] = "Row $row_number | 10-268: 'CONTRACT PHASE' IN (AC,CL,CA) AND 'MONTHLY PAYMENT AMOUNT' IS EMPTY OR 0";
        }
        ...
        if ($is_start_date_valid && $is_next_payment_date_valid && self::isDateGreaterThan($contract_start_date, $next_payment_date)) {
            $errors[] = "Row $row_number | 10-262: 'CONTRACT START DATE' IS GREATER THAN 'NEXT PAYMENT DATE'";
        }
        ...
        if (!empty($next_payment_date) && !$is_next_payment_date_valid || ($are_all_dates_valid && 
            ...
            $errors[] = "Row $row_number | 10-166: FIELD 'NEXT PAYMENT DATE' IS NOT CORRECT";
        }

        if (strlen($outstanding_payments_number) !== 0 && 
            (!ctype_digit($outstanding_payments_number) || strlen($outstanding_payments_number) > 3)) {
            $errors[] = "Row $row_number | 10-168: FIELD 'OUTSTANDING PAYMENTS NUMBER' IS NOT NUMERIC OR LENGTH IS NOT CORRECT";
        }
        if (in_array($contract_phase, ['CL', 'CA']) && !empty($outstanding_payments_number)) {
            $errors[] = "Row $row_number | 'CONTRACT PHASE' IN (CL,CA) AND 'OUTSTANDING PAYMENTS NUMBER' IS NOT EMPTY";
        }
        ...
        if (!empty($outstanding_payments_number) && empty($outstanding_balance) ||
            ...
            $errors[] = "Row $row_number | FIELDS 'OUTSTANDING PAYMENTS NUMBER' AND 'OUTSTANDING BALANCE' " . 
            "MUST EITHER BOTH BE EMPTY OR FILLED IN";
        }

        if (!ctype_digit($overdue_payments_number) || strlen($overdue_payments_number) > 15) {
            $errors[] = "Row $row_number | 10-171: FIELD 'OVERDUE PAYMENTS AMOUNT' IS NOT NUMERIC OR LENGTH IS NOT CORRECT";
        }
        if (in_array($contract_phase, ['RQ', 'RN', 'RF']) && !empty($overdue_payments_number)) {
            $errors[] = "Row $row_number | 'CONTRACT PHASE' IN (RE,RN,RQ) AND 'OVERDUE PAYMENTS NUMBER' IS NOT EMPTY";
        }
        ...
        if (!empty($overdue_payments_number) && empty($overdue_payments_amount) ||
            ...
            $errors[] = "Row $row_number | FIELDS 'OVERDUE PAYMENTS NUMBER' AND 'OVERDUE PAYMENTS AMOUNT' " . 
            "MUST EITHER BOTH BE EMPTY OR FILLED IN";
        }

        if (in_array($contract_phase, ['AC', 'CL', 'CA']) && strlen($overdue_days) === 0) {
            $errors[] = "Row $row_number | 20-090: IF 'CONTRACT PHASE' IN (AC,CL,CA), 'OVERDUE DAYS' IS MANDATORY";
        }
        if (!empty($overdue_days) && !in_array($overdue_days, self::OVERDUE_DAYS_DOMAIN)) {
            $errors[] = "Row $row_number | 10-172: FIELD 'OVERDUE DAYS' IS NOT CORRECT";
        }
        ...
        if (in_array($contract_phase, ['CL', 'CA']) && !empty($next_payment_date)) {
            $errors[] = "Row $row_number | 10-259: 'CONTRACT PHASE' IN (CL,CA) AND 'NEXT PAYMENT DATE' IS NOT EMPTY";
        }

        if ($is_end_actual_date_valid && $is_next_payment_date_valid && 
            self::isDateGreaterThan($next_payment_date, $contract_end_actual_date)) {
            $errors[] = "Row $row_number | 10-260: 'CONTRACT END ACTUAL DATE' IS LESS THAN 'NEXT PAYMENT DATE'";
        }

        if ($is_end_planned_date_valid && $is_next_payment_date_valid && 
            self::isDateGreaterThan($next_payment_date, $contract_end_planned_date)) {
            $errors[] = "Row $row_number | 10-261: 'CONTRACT END PLANNED DATE' IS LESS THAN 'NEXT PAYMENT DATE'";
        }
        ...
        if ($contract_phase === 'AC' && empty($outstanding_payments_number) &&
            ...
            $errors[] = "Row $row_number | 10-270: 'CONTRACT PHASE' IN (AC) AND 'OUTSTANDING PAYMENTS NUMBER' IS EMPTY " .
        ...
        if ($contract_phase === 'AC' && 
            ...
            $errors[] = "Row $row_number | 10-274: 'CONTRACT PHASE' IN (AC) AND 'OUTSTANDING PAYMENTS NUMBER' = (0 OR EMPTY) " . 
        ...
        if (in_array($contract_phase, ['AC', 'CL', 'CA']) &&
            ...
            $errors[] = "Row $row_number | 10-280: 'CONTRACT PHASE' IN (AC,CL,CA) AND 'OVERDUE PAYMENTS NUMBER' = (0 OR EMPTY) " . 
        ...
        if (in_array($contract_phase, ['AC', 'CL', 'CA']) && strlen($last_payment_amount) === 0) {
            $errors[] = "Row $row_number | 20-089: IF 'CONTRACT PHASE' IN (AC,CL,CA), 'LAST PAYMENT AMOUNT' IS MANDATORY";
        }
        ...
        if ((int) $last_payment_amount < 0) {
            $errors[] = "Row $row_number | 20-201: FIELD 'LAST PAYMENT AMOUNT' CAN'T HAVE A NEGATIVE VALUE";
        }
        ...
        if ((int) $overdue_payments_number < 0) {
            $errors[] = "Row $row_number | 20-207: FIELD 'OVERDUE PAYMENTS NUMBER' CAN'T HAVE A NEGATIVE VALUE";
        }
        ...
        if (in_array($contract_phase, ['AC', 'CL', 'CA']) && 
            ...
            $errors[] = "Row $row_number | 20-228: 'CONTRACT PHASE' IN (AC,CL,CA) AND 'OUTSTANDING PAYMENTS NUMBER' > 0 " . 
        ...
        if (in_array($contract_phase, ['AC', 'CL', 'CA']) && 
            ...
            $errors[] = "Row $row_number | 20-229: 'CONTRACT PHASE' IN (AC,CL,CA) AND 'OVERDUE PAYMENTS NUMBER' > 0 " . 
