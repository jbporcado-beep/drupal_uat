...
class ContactValidator {
    ...
    public function validate(ContactDto $contactDto, string $provider_subj_no, array &$errors, int $row_number): void {
        ...
        if (strlen($contact_1_type) !== 0 && !in_array($contact_1_type, array_keys(self::CONTACT_TYPE_DOMAIN))) {
            $errors[] = "$provider_subj_no | Row $row_number | 10-014: FIELD 'CONTACT 1 TYPE' IS NOT CORRECT";
        }
        if (strlen($contact_2_type) !== 0 && !in_array($contact_2_type, array_keys(self::CONTACT_TYPE_DOMAIN))) {
        ...
        if ((strlen($contact_1_type) !== 0 && strlen($contact_1_value) === 0) ||
            (strlen($contact_1_type) === 0 && strlen($contact_1_value) !== 0)) {
        ...
        if ((strlen($contact_2_type) !== 0 && strlen($contact_2_value) === 0) ||
            (strlen($contact_2_type) === 0 && strlen($contact_2_value) !== 0)) {
            $errors[] = "$provider_subj_no | Row $row_number | 10-074: FIELDS 'CONTACT 2 TYPE' AND 'CONTACT 2 VALUE' MUST " . 
                        "EITHER BOTH BE EMPTY OR FILLED IN";
        }

        if (strlen($contact_1_type) !== 0 && ($contact_1_type === $contact_2_type)) {
            $errors[] = "$provider_subj_no | Row $row_number | 10-071: MORE THAN ONE 'CONTACT TYPE' WITH THE SAME VALUE ARE NOT ALLOWED";
        }

        if (strlen($contact_1_type) === 0 && strlen($contact_2_type) === 0) {
