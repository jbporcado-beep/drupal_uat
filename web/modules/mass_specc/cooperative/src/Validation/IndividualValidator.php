...
class IndividualValidator {
    ...
    public function validate(IndividualDto $individualDto, array &$errors, int $row_number): void {
        ...
        if (!ctype_digit($title) || $title < 10 || $title > 21) {
            $errors[] = "Row $row_number | FIELD 'TITLE' IS NOT CORRECT";
        }

        if (empty($first_name)) {
            $errors[] = "Row $row_number | 20-002: FIELD 'FIRST NAME' IS MANDATORY";
        }
        if (strlen($first_name) > 70) {
            $errors[] = "Row $row_number | FIELD 'FIRST NAME' LENGTH MUST HAVE A LENGTH <= 70";
        }
        if (empty($last_name)) {
            $errors[] = "Row $row_number | 20-003: FIELD 'LAST NAME' IS MANDATORY";
        }
        if (strlen($last_name) > 70) {
            $errors[] = "Row $row_number | FIELD 'LAST NAME' LENGTH MUST HAVE A LENGTH <= 70";
        }
        if (strlen($middle_name) > 70) {
            $errors[] = "Row $row_number | FIELD 'MIDDLE NAME' LENGTH MUST HAVE A LENGTH <= 70";
        }
        if (strlen($suffix) > 40) {
            $errors[] = "Row $row_number | FIELD 'SUFFIX' LENGTH MUST HAVE A LENGTH <= 40";
        }
        if (strlen($previous_last_name) > 70) {
            $errors[] = "Row $row_number | FIELD 'PREVIOUS LAST NAME' LENGTH MUST HAVE A LENGTH <= 70";
        }

        if (empty($gender)) {
            $errors[] = "Row $row_number | 20-001: FIELD 'GENDER' IS MANDATORY";
        }
        if (!empty($gender) && strtoupper($gender) !== "M" && strtoupper($gender) !== "F") {
            $errors[] = "Row $row_number | 10-004: FIELD 'GENDER' IS NOT CORRECT";
        }

        if (empty($date_of_birth)) {
            $errors[] = "Row $row_number | 20-004: FIELD 'DATE OF BIRTH' IS MANDATORY";
        }
        if (!self::isValidDate($date_of_birth)) {
            $errors[] = "Row $row_number | 10-005: FIELD 'DATE OF BIRTH' IS NOT CORRECT";
        }
        if (!self::isValidAge($date_of_birth)) {
            $errors[] = "Row $row_number | 20-137: INDIVIDUAL AGE SHOULD BE BETWEEN 18 AND 100 YEARS";
        }   
        if (!self::isDateGreaterThan1900($date_of_birth)) {
            $errors[] = "Row $row_number | 20-125: FIELD 'DATE OF BIRTH' MUST BE GREATER THAN 01-01-1900";
        }

        if (!empty($place_of_birth) && strlen($place_of_birth) > 100) {
            $errors[] = "Row $row_number | FIELD 'PLACE OF BIRTH' LENGTH MUST HAVE A LENGTH <= 100";
        }

        if (!empty($country_of_birth_code) && !in_array(strtoupper($country_of_birth_code), self::COUNTRY_DOMAIN)) {
            $errors[] = "Row $row_number | 10-006: FIELD 'COUNTRY OF BIRTH' IS NOT CORRECT";
        }

        if (!empty($nationality) && !in_array(strtoupper($nationality), self::COUNTRY_DOMAIN)) {
            $errors[] = "Row $row_number | 20-040: FIELD 'NATIONALITY' IS NOT CORRECT";
        }

        if (!empty($resident) && $resident !== '0' && $resident !== '1') {
            $errors[] = "Row $row_number | FIELD 'RESIDENT' IS NOT CORRECT";
        }

        if (!empty($civil_status) && !in_array($civil_status, ['1', '2', '3', '4'])) {
            $errors[] = "Row $row_number | 20-042: FIELD 'CIVIL STATUS' IS NOT CORRECT";
        }

        if (!empty($number_of_dependents) && !ctype_digit($number_of_dependents) || $number_of_dependents > 99) {
            $errors[] = "Row $row_number | FIELD 'NUMBER OF DEPENDANTS' IS NOT CORRECT";
        }

        if (!empty($cars_owned) && !ctype_digit($cars_owned) || $cars_owned > 999) {
            $errors[] = "Row $row_number | FIELD 'CARS OWNED' IS NOT CORRECT";
        }
    }
}
