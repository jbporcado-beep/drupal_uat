...
class FamilyValidator {
    ...
    public function validate(FamilyDto $familyDto, string $provider_subj_no, array &$errors, int $row_number): void {
        ...
        foreach ($fields as $field_name => $field_value) {
            if (in_array($field_name, [
                ...
                $errors[] = "$provider_subj_no | Row $row_number | FIELD '$normalized_field_name' LENGTH MUST HAVE A LENGTH <= 70";
            }
            else if ($field_name === 'fatherSuffix' && strlen($field_value) > 40) {
                $errors[] = "$provider_subj_no | Row $row_number | FIELD 'FATHER SUFFIX' LENGTH MUST HAVE A LENGTH <= 40";
            }
        }
    }
}
