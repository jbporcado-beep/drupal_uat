...
class AddressValidator {
    ...
    public function validate(AddressDto $addressDto, array &$errors, int $row_number, string $record_type): void {
        ...
        if ($record_type === 'ID') {
            if ($address_1_address_type !== 'MI' && $address_2_address_type !== 'MI') {
                $errors[] = "Row $row_number | 20-142: AT LEAST ONE OCCURENCE BETWEEN ALL INDIVIDUAL ADDRESSES " . 
                            "MUST BE 'MAIN ADDRESS'";
            }
            if ($address_1_address_type !== 'MI' || $address_2_address_type !== 'AI') {
                $errors[] = "Row $row_number | 10-007: ADDRESS 1 TYPE MUST BE 'MI' AND ADDRESS 2 TYPE MUST BE 'AI'";
            }
            if (empty($address_1_address_type) || empty($address_1_fulladdress) || 
                empty($address_2_address_type) || empty($address_2_fulladdress)) {
                $errors[] = "Row $row_number | 20-130: ALL 'ADDRESS:TYPE' MUST BE FILLED IN. " .
            ...
            if ($address_1_address_type !== 'MT' || (!empty($address_2_address_type) && $address_2_address_type !== 'AT')) {
                $errors[] = "Row $row_number | 10-007: ADDRESS 1 TYPE MUST BE 'MT' AND ADDRESS 2 TYPE MUST BE 'AT'";
            }
        }

        if ((empty($address_1_address_type) && !empty($address_1_fulladdress)) ||
            (!empty($address_1_address_type) && empty($address_1_fulladdress))) {
            $errors[] = "Row $row_number | FIELDS 'ADDRESS 1 TYPE' AND 'ADDRESS 1 FULLADDRESS' " .
        ...
        if (strlen($address_1_fulladdress) > 400 || strlen($address_2_fulladdress) > 400) {
            $errors[] = "Row $row_number | FIELD 'ADDRESS: FULL ADDRESS' LENGTH MUST HAVE A LENGTH <= 400";  
        }
    }
}
?>